    ...
    try {
      ...
      const options = {
        ...
        handler: async (response: { razorpay_payment_id: string; razorpay_order_id: string; razorpay_signature: string }) => {
          ...
          if (verified) {
            await placeOrder(paymentId, "RAZORPAY");
          } else {
            toast.error("Payment verification failed");
          }
        },
        prefill: { name: address.name, contact: address.phone, email: guestEmail },
        theme: { color: "#C9956C" },
      };
    ...
    <div className="pt-32 pb-24 section-padding">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <motion.div key="address" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                  ...
                  <Input label="Guest Email (if not signed in)" type="email" placeholder="your@email.com" value={guestEmail} onChange={(e) => setGuestEmail(e.target.value)} />

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input label="Full Name *" value={address.name} onChange={(e) => setAddress(p => ({ ...p, name: e.target.value }))} required placeholder="Ayesha Rahman" />
                    <Input label="Phone Number *" value={address.phone} onChange={(e) => setAddress(p => ({ ...p, phone: e.target.value }))} required placeholder="+92 300 0000000" />
                  </div>
                  <Input label="Address Line 1 *" value={address.line1} onChange={(e) => setAddress(p => ({ ...p, line1: e.target.value }))} required placeholder="House #, Street #" />
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <Input label="City *" value={address.city} onChange={(e) => setAddress(p => ({ ...p, city: e.target.value }))} required placeholder="Lahore" />
                    <Input label="Province *" value={address.state} onChange={(e) => setAddress(p => ({ ...p, state: e.target.value }))} required placeholder="Punjab" />
                    <Input label="Postal Code *" value={address.pincode} onChange={(e) => setAddress(p => ({ ...p, pincode: e.target.value }))} required placeholder="54000" />
                  </div>
                ...
                <motion.div key="payment" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border border-ivory-200 p-5">
                      ...
                      <p className="font-inter text-xs text-mauve mb-4">Pay when your order arrives. Available in all major cities.</p>
                      <Button
                        variant="outline"
                        onClick={() => setStep(3)}
                        className="w-full"
              ...
              {step === 3 && (
                <motion.div key="review" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                  ...
                  <div className="border border-ivory-200 p-5 space-y-3">
                    <h3 className="font-inter text-xs tracking-widest uppercase text-charcoal-light mb-4">Delivery Address</h3>
                  ...
                  <Button
                    onClick={() => placeOrder(undefined, undefined)}
                    loading={loading}
                    className="w-full"
                  >
          ...
          {/* Order Summary */}
          <div>
            <div className="sticky top-28 border border-ivory-200 p-6">
              <h3 className="font-playfair text-xl text-charcoal mb-6">Order Summary</h3>
              ...
              <div className="border-t border-ivory-200 pt-4 space-y-3">
                ...
                {shipping === 0 && (
                  <p className="text-xs font-inter text-rose-gold">Free shipping on orders over ₹2,999</p>
                )}
