    ...
    <div className="min-h-screen bg-ivory flex">
      <div className="hidden lg:flex lg:flex-1 bg-charcoal-dark relative overflow-hidden items-center justify-center">
        ...
        <div className="relative z-10 text-center px-12">
          <h1 className="font-playfair text-5xl text-ivory mb-4">Azalea <span className="text-rose-gold italic">by Zehra</span></h1>
          ...
          <p className="mt-8 font-inter text-xs text-ivory/30 max-w-xs leading-relaxed">India&apos;s premier destination for exquisite ethnic wear</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <div className="w-full max-w-sm">
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="font-inter text-sm text-charcoal-light mb-8">Sign in to your Azalea by Zehra account</p>

            {/* OTP / Password toggle */}
            <div className="flex border border-ivory-200 mb-8">
              {(["otp", "password"] as AuthMode[]).map((m) => (
                <button key={m} type="button" onClick={() => switchMode(m)}
                  className={`flex-1 py-2.5 font-inter text-xs tracking-widest uppercase transition-all duration-200 ${mode === m ? "bg-charcoal text-ivory" : "text-charcoal-light hover:text-charcoal"}`}>
                ...
                <motion.div key="otp-mode" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  {otpStep === "contact" ? (
                    ...
                    <div className="space-y-8">
                      <p className="font-inter text-sm text-charcoal-light">We sent a 6-digit code to <span className="text-charcoal font-medium">{contact}</span></p>
                      <OTPInput value={otp} onChange={setOtp} />
                      <Button onClick={handleVerifyOTP} loading={loading} className="w-full" disabled={otp.length !== 6}>Verify & Sign In</Button>
                      <div className="flex items-center justify-between">
                        <button onClick={() => { setOtpStep("contact"); setOtp(""); }}
                          className="font-inter text-sm text-charcoal-light hover:text-charcoal transition-colors">← Change contact</button>
                        <button onClick={handleSendOTP} disabled={loading}
                          className="font-inter text-sm text-rose-gold hover:text-rose-gold-dark transition-colors disabled:opacity-50">Resend Code</button>
                      </div>
                    </div>
                  )}
                </motion.div>
